...
const brands = ['Tecno', 'Sony', 'Samsung', 'Apple', 'Xiaomi', 'Oppo', 'Vivo', 'Asus', 'Infinix']
const ramOptions = [4, 6, 8, 12, 16, 32]

// Watch for changes and emit
watch(filters, (newVal) => {
  const activeFilters = {}
  
  // Format arrays for query params (can be array or comma-separated depending on backend, using array format typical for axios `brand[]=Tecno&brand[]=Sony`)
  if (newVal.brand.length > 0) activeFilters.brand = newVal.brand
  if (newVal.ram.length > 0) activeFilters.ram = newVal.ram
  if (newVal.min_price) activeFilters.price_min = newVal.min_price
  if (newVal.max_price) activeFilters.price_max = newVal.max_price
...
<template>
  <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm sticky top-24">
    <div class="flex items-center justify-between mb-6">
      ...
      <button 
        @click="filters = { brand: [], ram: [], min_price: null, max_price: null }"
