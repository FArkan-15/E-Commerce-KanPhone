...
const fetchProducts = async () => {
  loading.value = true
  try {
    // Kita tambahkan has_discount: true ke API, dan juga filter manual di frontend untuk memastikan
    const data = await getProducts({ ...props.filters, has_discount: true })
    const allProducts = data.data || []
    products.value = allProducts.filter(item => item.discount && item.discount > 0)
  } catch (error) {
...
<template>
  <div class="flex-1 w-full">
    ...
    <div class="flex justify-between items-center mb-6 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
      <h1 class="text-xl font-bold text-gray-900">Special Promos</h1>
