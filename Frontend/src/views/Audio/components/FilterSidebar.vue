...
const brands = ['Moondrop', 'JBL', 'Samsung', 'Apple', 'Xiaomi']
const types = [
  { label: 'Headphones', value: 'headphone' },
  { label: 'In-Ear Monitor', value: 'iem' },
  { label: 'TWS', value: 'tws' },
...
<template>
  <div class="w-full md:w-[260px] lg:w-[280px] flex-shrink-0">
    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm sticky top-24">
      ...
      <!-- Connectivity -->
      <div class="mb-2">
        <h3 class="font-semibold text-sm mb-3 text-gray-800">Connectivity</h3>
        <div class="space-y-2.5 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
          <label v-for="conn in connectivities" :key="conn.value" class="flex items-center gap-3 cursor-pointer group">
            <div class="relative flex items-center justify-center">
              <input 
                type="checkbox" 
                :value="conn.value" 
            ...
            <span class="text-sm text-gray-600 group-hover:text-gray-900 transition-colors">{{ conn.label }}</span>
          </label>
        </div>
      </div>
