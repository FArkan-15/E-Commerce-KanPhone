...
const types = ['Case', 'Charger', 'Cable', 'Screen Protector', 'Power Bank', 'Audio', 'Mounts & Stands', 'Other']
...
<template>
  <div class="w-full md:w-[260px] lg:w-[280px] flex-shrink-0">
    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm sticky top-24">
      ...
      <!-- Price -->
      <div class="mb-6">
        <h3 class="font-semibold text-sm mb-3 text-gray-800">Price Range</h3>
      ...
      <div class="mb-6">
        ...
        <div class="space-y-2.5 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
          <label v-for="type in types" :key="type" class="flex items-center gap-3 cursor-pointer group">
            ...
            <span class="text-sm text-gray-600 group-hover:text-gray-900 transition-colors">{{ type }}</span>
          </label>
        </div>
      </div>
